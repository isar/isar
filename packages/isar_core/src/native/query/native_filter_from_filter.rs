...
pub fn native_filter_from_filter(
    filter: &Filter,
    collection: &NativeCollection,
    all_collections: &[NativeCollection],
) -> Option<NativeFilter> {
    let native_filter = match filter {
        Filter::Condition(condition) => condition_to_native(condition, collection)?,
        Filter::Json(json) => NativeFilter::json(
            collection.get_property(json.condition.property_index)?,
            json.path.clone(),
            json.condition.condition_type,
            json.condition.values.clone(),
            json.condition.case_sensitive,
        )?,
        Filter::Embedded(embedded) => {
            let property = collection.get_property(embedded.property_index)?;
            let embedded_collection_index = property.embedded_collection_index?;
            let embedded_collection = &all_collections[embedded_collection_index as usize];
            let filter =
                native_filter_from_filter(&embedded.filter, embedded_collection, all_collections)?;
            NativeFilter::embedded(property, filter)?
        }
        Filter::And(filters) => {
            // if any of the filters are None, return None
            let filters: Option<Vec<NativeFilter>> = filters
                .iter()
                .map(|f| native_filter_from_filter(f, collection, all_collections))
                .collect();
        ...
        Filter::Or(filters) => {
            ...
            let filters = filters
                .iter()
                .filter_map(|f| native_filter_from_filter(f, collection, all_collections))
                .collect();
            NativeFilter::or(filters)
        }
        Filter::Not(filter) => {
            let filter = native_filter_from_filter(filter, collection, all_collections);
...
fn condition_to_native(
    ...
    let filter = match condition.condition_type {
        ...
        ConditionType::StringRegex => {
            let value = condition.values.get(0)?.as_ref()?.string()?;
            NativeFilter::string_regex(property?, value, condition.case_sensitive)?
        }
        ConditionType::In => {
            NativeFilter::is_in(property?, &condition.values, condition.case_sensitive)
        }
    };
    Some(filter)
}
